...
const EXTRA_POI_ICON = icons.redCrossAdditional ?? icons.greySquare
 ...
 */
function buildLayerLabel(label, iconUrl) {
    ...
    return (
        ...
        `<img class="poi-overlay-icon" src="${safeUrl}" alt="">` +
        `</span>`
    )
}
...
async function buildOverlayMaps() {
    ...
    const results = await Promise.all(
        files.map(async (fileName) => {
            ...
            const isBase = key in BASE_POI_CONFIG

            const { label, icon } = isBase
                ? BASE_POI_CONFIG[key]
                : { label: stem.charAt(0).toUpperCase() + stem.slice(1), icon: EXTRA_POI_ICON }
            ...
            try {
                ...
                return { key, label, layer, isBase, iconUrl: icon.options?.iconUrl ?? null }
            } catch (error) {
    ...
    const toOverlayMap = (entries) =>
        Object.fromEntries(
            entries.map((e) => [buildLayerLabel(e.label, e.iconUrl), e.layer])
        )
...
// Kept at module scope so they can be removed when re-initialising after a
// config change triggers a full page reload.
...
export function initPoiOverlay(map) {
    buildOverlayMaps().then(({ baseOverlays, extraOverlays }) => {
        // Remove stale controls before adding the new ones.
